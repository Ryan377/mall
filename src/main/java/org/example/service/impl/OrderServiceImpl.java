...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderModel createOrder(Integer userId, Integer itemId,Integer promoId, Integer amount) throws BussinessException {
        ...
        if(amount<=0||amount>99)
            throw new BussinessException(EnumBussinessError.PARAMETER_VALIDATION_ERROR,"数量信息不存在");
        //校验活动信息
        if(promoId!=null){
            //1.校验对应活动是否适用于该商品
            if(promoId.intValue()!=itemModel.getPromoModel().getId()){
                throw new BussinessException(EnumBussinessError.PARAMETER_VALIDATION_ERROR,"活动信息不存在");
                //2.校验活动是否在进行中
            }else if (itemModel.getPromoModel().getStatus()!=2){
                throw new BussinessException(EnumBussinessError.PARAMETER_VALIDATION_ERROR,"活动还未开始");
            }
        }
